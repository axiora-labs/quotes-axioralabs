...
// --- NEW: HTML to Plain Text Converter ---
...
export const generateProfessionalPDF = async (data: InvoiceData) => {
  ...
  // --- 8. NOTES & TERMS (FIXED: Now converts HTML to plain text) ---
  finalY += 10; 

  if (data.notes) {
    // Convert HTML to plain text
    const plainNotes = htmlToPlainText(data.notes);
  ...
  if (data.terms) {
    // Convert HTML to plain text
    const plainTerms = htmlToPlainText(data.terms);
    const splitTerms = doc.splitTextToSize(plainTerms, 180);
